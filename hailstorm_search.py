...
def recTree(root, depth, memo, seach_val, seq=""):
    if root.value == search_val: print(seq)
    if depth == 0: return
    right_val = -1
    ...
    if right_val in memo:
        ...
        root.right = right_leaf
        recTree(right_leaf, depth-1, memo, search_val, seq+"R")
    ...
    if (root.value * 2) in memo:
        ...
        root.left = left_leaf
        recTree(left_leaf, depth-1, memo, search_val, seq+"L")
...
search_val = input("Find what? ")
search_val = int(search_val) if len(search_val)>0 else 6

memo = {}
root = Tree(root_val)
recTree(root, depth, memo,search_val)
#prntTree(root, depth)
#print(root.to_str())
